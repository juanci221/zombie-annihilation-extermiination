...
class MainSceneA6 extends Phaser.Scene {
    constructor() {
        super({ key: 'MainSceneA6' });
        this.player = null;
        this.cursors = null;
        this.monedas = null;
 ...
 this.scale.scaleMode = Phaser.Scale.SHOW_ALL;
  ...
  this.scale.pageAlignVertically = true;
   this.load.image('Fon0', 'tiled/Fon0.png');
   this.load.image('Fon1', 'tiled/Fon1.png');
   this.load.image('Fon2', 'tiled/Fon2.png');
   
   this.load.image('Fon3', 'tiled/Fon3.png');
   this.load.image('Fon4', 'tiled/Fon4.png');
   this.load.image('Fon5', 'tiled/Fon5.png');
   
   
   this.load.image('moneda', 'imag/derbala1.png');
    
  this.load.image('soporte', 'imag/molino1.png'); 

   this.load.spritesheet('animacionVeneno1', 'imag/molino.png', {
        frameWidth: 400,  // Ancho de un solo frame
        frameHeight: 320 // Alto de un solo frame
    });
...
this.load.spritesheet('Ee', 'imag/Zg5.png', { frameWidth: 300, frameHeight: 370 });

        this.load.tilemapTiledJSON('mapa02','assets/mapas/mapa2.json');
   
this.load.spritesheet('dude', 'imag/playerC231.png', { frameWidth: 120, frameHeight: 130 });
 
 
    this.load.spritesheet('ataque2', 'imag/S3.png', { frameWidth: 120, frameHeight: 100 });
  ...
  // this.load.spritesheet('E', 'imag/players2.png', { frameWidth: 125, frameHeight: 150 });
   this.load.spritesheet('A', 'imag/ataque4.png', { frameWidth: 100, frameHeight: 100 });
   ...
   this.load.spritesheet('bala', 'imag/bala11.png', { frameWidth: 100, frameHeight: 130 });
...
playerInvulnerable = false; 
  ...
  that = this; // Almacena 'this' en 'that'
   mapa02 = this.make.tilemap({ key: 'mapa02'});

  const Fon0 = mapa02.addTilesetImage('Fon0');
  const Fon1 = mapa02.addTilesetImage('Fon1');
  const Fon2 = mapa02.addTilesetImage('Fon2');
  const Fon3 = mapa02.addTilesetImage('Fon3' );
  const Fon4 = mapa02.addTilesetImage('Fon4' );
  const Fon5 = mapa02.addTilesetImage('Fon5' );
  const layer = mapa02.createLayer("Fon0", Fon0);
  const layer1 = mapa02.createLayer("Fon1", Fon1);
  const layer2 = mapa02.createLayer("Fon2", Fon2);
  const layer5 = mapa02.createLayer("Fon3", Fon3);
  const layer6 = mapa02.createLayer("Fon4", Fon4);
  const layer7 = mapa02.createLayer("Fon5", Fon5);
...
const mapWidth = mapa02.widthInPixels;
...
//fogOverlay.fillStyle(0xff0000, 0.3); // Rojo con 20% de opacidad
...
this.sound.stopAll(); // Detiene todos los sonidos, incluida la música de fondo
  ...
  const musicaDeFondoA = this.sound.add('fa', { volume: 0.5, loop: true });
    musicaDeFondoA.play();
...
this.physics.world.setBounds(0, 0, mapa02.widthInPixels, mapa02.heightInPixels);
...
player.barraSalud.setScale(0.6);
 player.setDepth(2);
this.sonidoPlayer = this.sound.add('up',{volume: 1});
...
player.textoSalud = this.add.text(barX, barY - 24, 'Health ' + player.salud, {
  ...
  this.cameras.main.setBounds(0, 0, mapa02.widthInPixels, mapa02.heightInPixels);
...
// --- Movimiento Izquierda (Más rápido) ---
if (!this.anims.exists('enemy22_left')) {
    this.anims.create({
      key: 'enemy22_left',
      frames: this.anims.generateFrameNumbers('Ee', { start: 0, end: 5 }),
      frameRate: 10, // ⬅️ Aumentado para un movimiento más rápido
      repeat: -1
    });
}
 
// --- Movimiento Derecha (Más rápido) ---
if (!this.anims.exists('enemy22_right')) {
    this.anims.create({
      key: 'enemy22_right',
      frames: this.anims.generateFrameNumbers('Ee', { start: 6, end: 11 }),
      frameRate: 10, // ⬅️ Aumentado para un movimiento más rápido
      repeat: -1
    });
}
...
if (!this.anims.exists('enemy22_up')) {
    this.anims.create({
      key: 'enemy22_up', 
      frames: this.anims.generateFrameNumbers('Ee', { start: 0, end: 11 }), 
      frameRate: 10, 
      repeat: -1 
    });
}
...
let pinchos = obstaculos.create(320, 140, 'soporte');

let pinchos0 = obstaculos.create(3430, 120, 'soporte');
let pinchos1 = obstaculos.create(4100, 120, 'soporte');
let pinchos2 = obstaculos.create(4820, 140, 'soporte');

// Haz que sea un objeto estático para que no se mueva con colisiones
pinchos.setImmovable(true); 
pinchos0.setImmovable(true); 
pinchos1.setImmovable(true); 
pinchos2.setImmovable(true); 



    // Define la animación (solo una vez)
    if (!this.anims.exists('gasVeneno1')) {
    this.anims.create({
        key: 'gasVeneno1',
        frames: this.anims.generateFrameNumbers('animacionVeneno1', { start: 0, end: 4 }), // Ajusta los frames según tu spritesheet
        frameRate: 10,  // Velocidad de la animación (10 cuadros por segundo)
        repeat: -1      // -1 para que se repita en un bucle infinito
    });
    }
...
obstaculos.children.entries.forEach(function (obstaculo) {
    ...
    let animacionGas = this.add.sprite(obstaculo.x, obstaculo.y, 'animacionVeneno1');
animacionGas.setAlpha(1); 
    // Reproduce la animación
    animacionGas.play('gasVeneno1');
 ...
 // enemigos 1
  var datosEnemigo2 = [
    ...
    {x: 2500, y: 500, imagen: 'enemi6b'},
      ...
      {x: 8000, y: 500, imagen: 'enemi6'},
    {x: 7500, y: 500, imagen: 'enemi6b'},
      {x: 7200, y: 500, imagen: 'enemi6'},
    {x: 7700, y: 500, imagen: 'enemi6b'},
      {x: 6900, y: 500, imagen: 'enemi6'},
    {x: 6500, y: 500, imagen: 'enemi6b'},
    
     // Añade enemigos al grupo
  ...
  for (var i = 0; i < datosEnemigo2.length; i++) {
    ...
    enemigo2.salud = 120; // Asigna la salud al enemigo
    enemigo2.setDepth(2);
...
var datosEnemigos = [
  ...
  {x: 3600, y: 600, imagen: 'enemi1'},
  {x: 1100, y: 600, imagen: 'enemi1a'},
  {x: 3000, y: 600, imagen: 'enemi2a'},
  {x: 3100, y: 600, imagen: 'enemi3'},
  {x: 2500, y: 300, imagen: 'enemi5'},
  {x: 2600, y: 600, imagen: 'enemi12'}, 
  {x: 3100, y: 600, imagen: 'enemi3a'}, 
  {x: 3800, y: 600, imagen: 'enemi8'},
  {x: 3500, y: 600, imagen: 'enemi9'},
  {x: 7400, y: 600, imagen: 'enemi10'},

  {x: 3500, y: 600, imagen: 'enemi11'},
  {x: 3500, y: 600, imagen: 'enemi4a'},
  {x: 7600, y: 600, imagen: 'enemi12'},
  {x: 3700, y: 600, imagen: 'enemi1a'},
  {x: 7000, y: 500, imagen: 'enemi1'},

 {x: 1600, y: 500, imagen: 'enemi2a'},
 {x: 7400, y: 250, imagen: 'enemi11'},
 {x: 5500, y: 600, imagen: 'enemi5a'},
 {x: 7600, y: 500, imagen: 'enemi12'},
  {x: 4900, y: 300, imagen: 'enemi1a'},
  {x: 5000, y: 300, imagen: 'enemi1'},

   {x: 7100, y: 500, imagen: 'enemi2a'},
 {x: 7400, y: 250, imagen: 'enemi11'},
 {x: 7500, y: 600, imagen: 'enemi5a'},
 {x: 7600, y: 500, imagen: 'enemi12'},
  {x: 7700, y: 300, imagen: 'enemi1a'},
  {x: 7700, y: 300, imagen: 'enemi1'},
{x: 7600, y: 600, imagen: 'enemi1'},
...
for (var i = 0; i < datosEnemigos.length; i++) {
  ...
  enemigo1.salud = 120; // Asigna la salud al enemigo
  enemigo1.setDepth(2);
...
{x: 5600, y: 500, imagen: 'enemiG'},
  {x: 4900, y: 600, imagen: 'enemiG6'},
  {x: 2500, y: 600, imagen: 'enemiG3'},
  {x: 2700, y: 600, imagen: 'enemiG4'},
  {x: 4000, y: 600, imagen: 'enemiG5'},
  {x: 600, y: 600, imagen: 'enemiG6'},
  {x: 2900, y: 600, imagen: 'enemiG1'},
  {x: 2000, y: 300, imagen: 'enemiG2'},
  {x: 3000, y: 500, imagen: 'enemiG5'},
  {x: 1200, y: 200, imagen: 'enemiG6'},

  {x: 900, y: 200, imagen: 'enemiG1'},
  {x: 5000, y: 300, imagen: 'enemiG5'},
 {x: 900, y: 300, imagen: 'enemiG1'},
  {x: 5000, y: 300, imagen: 'enemiG5'},

  
  {x: 4000, y: 600, imagen: 'enemiG'},
  {x: 1900, y: 500, imagen: 'enemiG6'},
  {x: 5000, y: 300, imagen: 'enemiG2'},
  {x: 4500, y: 300, imagen: 'enemiG3'},
  {x: 3800, y: 600, imagen: 'enemiG4'},
 
 {x: 7600, y: 500, imagen: 'enemiG'},
  {x: 7900, y: 500, imagen: 'enemiG6'},
  {x: 7500, y: 500, imagen: 'enemiG3'},
  {x: 7700, y: 500, imagen: 'enemiG4'},
  {x: 8000, y: 500, imagen: 'enemiG5'},
  {x: 7600, y: 500, imagen: 'enemiG6'},
  {x: 7900, y: 500, imagen: 'enemiG1'},
  {x: 7800, y: 300, imagen: 'enemiG2'},
  {x: 7800, y: 500, imagen: 'enemiG5'},
  {x: 7500, y: 600, imagen: 'enemiG6'},
...
var datosEnemigosZ1 = [
  ...
  {x: 800, y: 600, imagen: 'ataque2'},
  {x: 900, y: 600, imagen: 'ataque2'},
  {x: 1000, y: 600, imagen: 'ataque2'},
  {x: 7500, y: 600, imagen: 'ataque2'},
  {x: 2000, y: 600, imagen: 'ataque2'},
  {x: 2100, y: 600, imagen: 'ataque2'},
  {x: 2150, y: 600, imagen: 'ataque2'},
  {x: 7600, y: 600, imagen: 'ataque2'},

  
  {x: 3000, y: 600, imagen: 'ataque2'},
  {x: 3100, y: 600, imagen: 'ataque2'},
  {x: 3150, y: 600, imagen: 'ataque2'},
  {x: 3200, y: 600, imagen: 'ataque2'},
  {x: 7700, y: 600, imagen: 'ataque2'},
  {x: 4000, y: 600, imagen: 'ataque2'},
  {x: 4100, y: 600, imagen: 'ataque2'},
  {x: 4150, y: 600, imagen: 'ataque2'},
  {x: 4200, y: 600, imagen: 'ataque2'},

  {x: 7600, y: 600, imagen: 'ataque2'},
  {x: 4600, y: 600, imagen: 'ataque2'},
  {x: 6500, y: 600, imagen: 'ataque2'},
  {x: 6550, y: 600, imagen: 'ataque2'},
  {x: 6600, y: 600, imagen: 'ataque2'},
  {x: 7750, y: 600, imagen: 'ataque2'},
  {x: 6850, y: 600, imagen: 'ataque2'},
  {x: 7800, y: 600, imagen: 'ataque2'},
...
// Crea el grupo de enemigos
enemigoZ1 = this.physics.add.group();

var sonidoMosc = this.sound.add('mosc', { volume: 5, loop: true }); 


// Crea varios enemigos y añádelos al grupo
for (var i = 0; i < datosEnemigosZ1.length; i++) {
  var enemigo100 = this.physics.add.sprite(datosEnemigosZ1[i].x, datosEnemigosZ1[i].y, datosEnemigosZ1[i].imagen);
  enemigo100.setScale(0.8);
  enemigo100.setCollideWorldBounds(true);
  enemigo100.impactos = 0; // Inicializa el conteo de impactos en 0
  enemigo100.salud = 100; // Asigna la salud al enemigo
  enemigo100.speed = 140; // Velocidad de movimiento
  enemigo100.setDepth(2);
  enemigoZ1.add(enemigo100);


 this.tweens.add({
        targets: enemigo100, // El tween se aplica a cada enemigo recién creado
        tint:0x00ffff ,    // El color al que cambiará (rojo)
        duration: 500,     // Duración de la animación en milisegundos
        yoyo: true,        // Hace que la animación se invierta
        repeat: -1,        // Repite el efecto infinitamente
...
enemigoZ1.getChildren().forEach(enemigo100 => {
  enemigo100.sonido = sonidoMosc;
  enemigo100.sonido.play();
});
...
var datosEnemigosZ2 = [
  ...
  {x: 950, y: 200, imagen: 'ataque3'},
  {x: 970, y: 200, imagen: 'ataque3'},
  {x: 990, y: 200, imagen: 'ataque3'},
  {x: 1000, y: 200, imagen: 'ataque3'},
  {x: 1100, y: 200, imagen: 'ataque3'},


  {x: 2000, y: 200, imagen: 'ataque3'},
  {x: 2100, y: 200, imagen: 'ataque3'},
  {x: 2150, y: 200, imagen: 'ataque3'},
  {x: 2200, y: 200, imagen: 'ataque3'},

  {x: 5450, y: 200, imagen: 'ataque3'},
  {x: 5400, y: 200, imagen: 'ataque3'},
  {x: 5350, y: 200, imagen: 'ataque3'},
  {x: 5650, y: 200, imagen: 'ataque3'},

  {x: 6700, y: 200, imagen: 'ataque3'},
  {x: 6750, y: 200, imagen: 'ataque3'},
  {x: 6800, y: 200, imagen: 'ataque3'},
  {x: 6850, y: 200, imagen: 'ataque3'},

  {x: 7500, y: 200, imagen: 'ataque3'},
  {x: 7650, y: 200, imagen: 'ataque3'},
  {x: 7700, y: 200, imagen: 'ataque3'},
  {x: 7750, y: 200, imagen: 'ataque3'},
 
];


// Crea el grupo de enemigos
enemigoZ2 = this.physics.add.group();




// Crea varios enemigos y añádelos al grupo
for (var i = 0; i < datosEnemigosZ2.length; i++) {
  var enemigoZa = this.physics.add.sprite(datosEnemigosZ2[i].x, datosEnemigosZ2[i].y, datosEnemigosZ2[i].imagen);
  enemigoZa.setScale(1);
  enemigoZa.setCollideWorldBounds(true);
  enemigoZa.impactos = 0; // Inicializa el conteo de impactos en 0
  enemigoZa.salud = 100; // Asigna la salud al enemigo
  enemigoZa.speed = 100; // Velocidad de movimiento
  enemigoZa.setDepth(2);
  enemigoZ2.add(enemigoZa);


  this.tweens.add({
        targets: enemigoZa, // El tween se aplica a cada enemigo recién creado
        tint:0x00ffff ,    // El color al que cambiará (rojo)
        duration: 500,     // Duración de la animación en milisegundos
        yoyo: true,        // Hace que la animación se invierta
        repeat: -1,        // Repite el efecto infinitamente
...
// Crea el grupo de enemigos
enemigoZ = this.physics.add.group();



// Crea varios enemigos y añádelos al grupo
for (var i = 0; i < datosEnemigosZ.length; i++) {
  var enemigo10 = this.physics.add.sprite(datosEnemigosZ[i].x, datosEnemigosZ[i].y, datosEnemigosZ[i].imagen);
  enemigo10.setScale(1.1);
  enemigo10.setCollideWorldBounds(true);
  enemigo10.impactos = 0; // Inicializa el conteo de impactos en 0
  enemigo10.speed = 160; // Velocidad de movimiento
  enemigo10.setDepth(2);
  enemigoZ.add(enemigo10);

   // --- NUEVO CÓDIGO AÑADIDO ---
    this.tweens.add({
        targets: enemigo10, // El tween se aplica a cada enemigo recién creado
        tint:0x00ffff ,    // El color al que cambiará (rojo)
        duration: 500,     // Duración de la animación en milisegundos
        yoyo: true,        // Hace que la animación se invierta
        repeat: -1,        // Repite el efecto infinitamente
...
totalEnemigos = enemigoZ2.countActive(true) + enemigoZ1.countActive(true) + enemigos.countActive(true) + enemigoss.countActive(true) + enemigosA.countActive(true); 

var sonidoSpat = this.sound.add('zombi', { volume: 7  });
spatEnemya = this.physics.add.sprite(7600, 600, 'Ee');
spatEnemya.setScale(1);
spatEnemya.salud = 3800;
spatEnemya.impactos = 0;
spatEnemya.damage = 15;
spatEnemya.salto = true;
spatEnemya.lastAttackTime = 0;
spatEnemya.attackCooldown = 2000;
spatEnemya.isAttacking = false;
spatEnemya.estado = 'idle';
spatEnemya.sonido = sonidoSpat;
spatEnemya.sonido.play();
totalEnemigos++;




crearPisoDeRebote(this, [ player, enemigos, enemigoss, enemigosA, spatEnemya], {
  y: 790,
  altura: 10,
  color: 0x00ff00,
  fuerzaRebote: -600,
...
this.time.addEvent({
  ...
  loop: true
    ...
    // Define las animaciones para el enemigo
    if (!this.anims.exists('enemy_left')) {
     this.anims.create({
         key: 'enemy_left',
         frames: this.anims.generateFrameNumbers('ataque', { start: 0, end: 4 }),
         frameRate: 10,
         repeat: -1
     });
   }
   
   if (!this.anims.exists('enemy_turn')) {
     this.anims.create({
         key: 'enemy_turn',
         frames: [
           { key: 'ataque', frame: 0 },
   ...
   if (!this.anims.exists('enemy_right')) {
     this.anims.create({
         key: 'enemy_right',
         frames: this.anims.generateFrameNumbers('ataque', { start: 5, end: 9 }),
         frameRate: 10,
         repeat: -1
     });
   }
...
if (!this.anims.exists('enemy1_left')) {
    this.anims.create({
        key: 'enemy1_left',
        frames: this.anims.generateFrameNumbers('ataque2', { start: 0, end: 4 }),
        frameRate: 10,
        repeat: -1
    });
  }
  
  if (!this.anims.exists('enemy1_turn')) {
    this.anims.create({
        key: 'enemy1_turn',
        frames: [
          { key: 'ataque2', frame: 0 },
  ...
  if (!this.anims.exists('enemy1_right')) {
    this.anims.create({
        key: 'enemy1_right',
        frames: this.anims.generateFrameNumbers('ataque2', { start: 5, end: 9 }),
        frameRate: 10,
        repeat: -1
    });
  }
...
if (!this.anims.exists('enemyA_left')) {
    this.anims.create({
        key: 'enemyA_left',
        frames: this.anims.generateFrameNumbers('ataque3', { start: 0, end: 4 }),
        frameRate: 10,
        repeat: -1
    });
  }
  
  if (!this.anims.exists('enemyA_turn')) {
    this.anims.create({
        key: 'enemyA_turn',
        frames: [
          { key: 'ataque3', frame: 0 },
  ...
  if (!this.anims.exists('enemyA_right')) {
    this.anims.create({
        key: 'enemyA_right',
        frames: this.anims.generateFrameNumbers('ataque3', { start: 5, end: 9 }),
        frameRate: 10,
        repeat: -1
    });
  }
...
if (!this.anims.exists('enemigoM_left_walk')) {
  const config1 = this.anims.create({
    key: 'enemigoM_left_walk',
    frames: this.anims.generateFrameNumbers('enemi',{ start: 0, end: 4 }),
    frameRate: 8,
    repeat: -1
  });
 }

 if (!this.anims.exists('enemigoM_right_walk')) {
  const config2 = this.anims.create({
  key: 'enemigoM_right_walk',
  frames: this.anims.generateFrameNumbers('enemi',{ start: 5, end: 9 }),
  frameRate: 8,
  repeat: -1
  });
 }
...
// Colisión de Balas contra Enemigos (solo se añade una vez)
...
this.physics.add.collider(balaHielo, enemigoZ1, this.handleBulletEnemyCollision, null, this);
this.physics.add.collider(balaHielo, enemigoZ2, this.handleBulletEnemyCollision, null, this);

if (this.bossOverlap) {
  this.physics.world.removeCollider(this.bossOverlap);
}
this.bossOverlap = this.physics.add.overlap(balaHielo, spatEnemya, this.handleBulletBossCollision, null, this);
...
this.ataqueEnemigoProyectil = (player, enemigo) => {
    // Verifica si el jugador y el enemigo todavía existen
    if (!player.active || !enemigo.active) {
        return;
    }

    // 1. Lógica de daño al jugador
    player.salud -= 10;
    player.setTint(0xff0000);
    this.time.addEvent({
        delay: 200,
        callback: () => restablecerColor(player),
        callbackScope: this
    });
 ...
 // this.sound.play('explosionSound');
  ...
  this.time.delayedCall(900, () => {
    this.scene.start('GameOverScene', { lostSceneKey: 'MainSceneA6' });
  });
}

};

// Asegúrate de que tu colisión se vea así:
this.physics.add.collider(player, enemigoZ, this.ataqueEnemigoProyectil, null, this);
...
this.physics.add.collider(player, enemigoZ2, impactos, null, this);
this.physics.add.collider(player, enemigoZ1, impactos, null, this);
this.physics.add.collider(player, spatEnemya, impactos, null, this);
...
this.physics.add.overlap(player, enemigoZ2, impactos, null, this);
this.physics.add.overlap(player, enemigoZ1, impactos, null, this);
this.physics.add.overlap(player, spatEnemya, impactos, null, this);





const objetoColision = mapa02.getObjectLayer('ObjetoColision');
   ...
   objetoColision.objects.forEach(function (obj) {
     ...
     this.physics.add.collider(enemigoZ2, objetoSprite, function() {
...
const objetoColisione = mapa02.getObjectLayer('ObjetoColisionG');
  
      objetoColisione.objects.forEach(function (obj) {
        // Crea un sprite para cada objeto en la capa de objetos
        var objetoSprites = this.physics.add.image(obj.x + obj.width / 2, obj.y + obj.height / 2, 'ruta1');
  ...
  this.physics.add.collider(enemigoZ1, objetoSprites, function() {
         
         }); 

           this.physics.add.collider(enemigoZ, objetoSprites, function() {
     
    });
    

          this.physics.add.collider(spatEnemya, objetoSprites, function() {
...
if (this.sys.game.device.os.desktop) {
  buttonShoot.setVisible(false); // fuerza a mostrarlo en PC
}
...
function crearPisoDeRebote(scene, objetosRebotables, opciones = {}) {
  ...
  const anchoMapa = mapa02?.widthInPixels || this.scale.width;
  ...
  objetosRebotables.forEach(objeto => {
    scene.physics.add.collider(objeto, pisoRebote, (entidad) => {
      if (entidad.body.velocity.y > 0) {
        ...
        // Sonido opcional
        if (scene.sound.get('rebote')) {
          scene.sound.play('rebote', { volume: 0.5 });
        }
      }
    }, null, scene);
  });
...
checkEnemigos() {
    ...
    const gruposDeEnemigos = [enemigoZ1, enemigoZ2, enemigos, enemigoss, enemigosA];
        ...
        if (grupo) { // Asegúrate de que el grupo existe
            grupo.children.iterate(enemigo => {
                if (enemigo) { // Asegúrate de que el enemigo existe
                    ...
                    if (enemigo.x < 0 || enemigo.x > mapa02.widthInPixels || enemigo.y < 0 || enemigo.y > mapa02.heightInPixels) {
...
eliminarEnemigo(enemigo) {
  ...
  enemigoZ1.remove(enemigo, true, true);
  enemigoZ2.remove(enemigo, true, true);
  totalEnemigos--;
}
...
handleBulletBossCollision(bala, jefe) {
  if (!bala || !jefe || !bala.active || !jefe.active || !jefe.body) return;
  ...
  // Aplicar daño
  jefe.impactos = (jefe.impactos || 0) + 1;
  jefe.salud -= daño;

  jefe.setTint(0xff0000);
  this.time.addEvent({
    delay: 100,
    callback: this.restablecerColoresA, // Referencia a la función
    callbackScope: this,              // Contexto de la escena
    args: [jefe]                      // Argumento de la función
});
  ...
  if (player && player.body) {
    ...
    jefe.body.setVelocityX(200 * direccion);
    jefe.body.setVelocityY(-80);
  }
  ...
  // Texto flotante
  const texto = this.add.text(jefe.x, jefe.y - 30, '.', {
    fontSize: '35px',
    fill: '#ff0000'
  });
  ...
  if (jefe.salud <= 0) {
    ...
    // Desactivar cuerpo y visibilidad
    jefe.setVisible(false);
    jefe.body.enable = false;
    jefe.active = false;

    // Eliminar minions si existen
    if (enemigoZ && enemigoZ.getChildren) {
      enemigoZ.getChildren().forEach(minion => {
        if (minion && minion.active) minion.destroy();
      });
    }

    // Eliminar colisión si fue registrada
    if (this.bossOverlap) {
      this.physics.world.removeCollider(this.bossOverlap);
    ...
    // Explosión visual
    const explosion = this.add.sprite(x, y, 'explosion_zombie');
    explosion.setScale(jefe.scaleX, jefe.scaleY);
    explosion.play('zombie_explosion');
    ...
    explosion.on('animationcomplete-zombie_explosion', () => {
      ...
      jefe.destroy();
      spatEnemya = null;
      totalEnemigos = Math.max(0, totalEnemigos - 1);
    });
  }
}
 ...
 restablecerColoresA(jefe) {
        if (jefe && jefe.active && jefe.body) {
            jefe.clearTint();
...
this.checkCoinsCollected();
  ...
  this.updatePlayerHealthBar();
   ...
   // 1. Llama al update del jefe, SOLO SI EXISTE.

   enemigoZ.getChildren().forEach((enemigo10, index) => { //  AGREGAR 'index'
        if (spatEnemya && enemigo10.active && !enemigo10.isThrown) { 
            // Pasar el 'index' a updateEnemyLogic
            this.updateEnemyLogic(enemigo10, spatEnemya, index); 
        }
    });

    enemigoZ1.getChildren().forEach(enemigo100 => {
    this.updateEnemyLogic1(enemigo100, player);
  });

 enemigoZ2.getChildren().forEach(enemigoZa => {
    this.updateEnemyLogic2(enemigoZa, player);
  });
...
const jumpSpeed = -800; // Ascenso más explosivo
...
checkCoinsCollected() {
    ...
    const isSpatEnemyDefeated = (spatEnemya === null || typeof spatEnemya === 'undefined');

    if (totalEnemigos === 0 && isSpatEnemyDefeated) {
        let elapsedTime = Math.floor((this.time.now - this.startTime) / 1000);
        
        this.scene.start('EndDemo', {
            score: this.score,
...
updateEnemies() {
  ...
  var velocidadEnemigo2 = 110;
  var distanciaDeteccion = 1100; // Distancia a la que el enemigo detecta al jugador
...
enemigosA.children.iterate(function (enemX) {
   ...
   var startY1 = enemD.y + enemD.height / 2 - 10;

                var damageTexto = this.add.text(enemD.x, startY1, '....', { alpha: 0.5, fontSize: '25px', fill: '#e1e7e2ff' });
                 damageTexto.setOrigin(0.5);
                // Añade la animación Tween para hacerlo flotar y desaparecer
                this.tweens.add({
                    targets: damageTexto,
...
// Itera sobre cada enemigo en el grupo de enemigos
  enemigoZ1.children.iterate(function (enemAs) {
    enemigoZ1.children.iterate(function (enemBs) {
      // No compara el enemigo consigo mismo
      if (enemAs !== enemBs) {
        // Calcula la distancia entre los dos enemigos
        var distancia = Phaser.Math.Distance.Between(enemAs.x, enemAs.y, enemBs.x, enemBs.y);
  
       // Si la distancia es menor que la distancia mínima y no se ha alcanzado el máximo de apilamiento
       if (distancia < distanciaMinima && contadorApilamiento < maxApilamiento) {
        // Hace que enem1 se suba a la mitad de enem2
        enemAs.y = enemBs.y - enemAs.height / 2;
        // Incrementa el contador de apilamiento
        contadorApilamiento++;
        }
      }
...
// Itera sobre cada enemigo en el grupo de enemigos
  enemigoZ2.children.iterate(function (enemAs1) {
    enemigoZ2.children.iterate(function (enemBs1) {
      // No compara el enemigo consigo mismo
      if (enemAs1 !== enemBs1) {
        // Calcula la distancia entre los dos enemigos
        var distancia = Phaser.Math.Distance.Between(enemAs1.x, enemAs1.y, enemBs1.x, enemBs1.y);
  
       // Si la distancia es menor que la distancia mínima y no se ha alcanzado el máximo de apilamiento
       if (distancia < distanciaMinima && contadorApilamiento < maxApilamiento) {
        // Hace que enem1 se suba a la mitad de enem2
        enemAs1.y = enemBs1.y - enemAs1.height / 2;
        // Incrementa el contador de apilamiento
        contadorApilamiento++;
        }
      }
...
updateJefe() {
  if (spatEnemya && spatEnemya.active && spatEnemya.body && spatEnemya.visible) {
    const distance = Phaser.Math.Distance.Between(player.x, player.y, spatEnemya.x, spatEnemya.y);
    const CHASE_RANGE = 800;
    const ATTACK_RANGE = 500;
    const currentTime = this.time.now;

    if (distance < CHASE_RANGE) {
      // --- ATAQUE DE ATRACCIÓN CON TENTÁCULOS ---
      if (distance < ATTACK_RANGE && !spatEnemya.isAttacking && currentTime > spatEnemya.lastAttackTime + spatEnemya.attackCooldown) {
        spatEnemya.body.setVelocityX(0);
        spatEnemya.lastAttackTime = currentTime;
        spatEnemya.isAttacking = true;
        spatEnemya.estado = 'atraccion';

        const graphics = this.add.graphics();
        const tentacleCount = 4;
        let alpha = 1;
        let fase = 1; // fase inicial: rápido

        const tentacleUpdate = this.time.addEvent({
          delay: 16,
          callback: () => {
            // Verificar que enemigo y jugador existan
            if (!spatEnemya || !spatEnemya.active || !spatEnemya.body || !player || !player.active) {
              tentacleUpdate.remove();
              graphics.destroy();
              return;
            }
            ...
            for (let i = 0; i < tentacleCount; i++) {
              // Negro con rojo
              const color = Phaser.Display.Color.GetColor(30, 0, 0 + i * 50);
              const thickness = 4 + i;
              graphics.lineStyle(thickness, color, alpha);

              const offsetX = spatEnemya.x + (i - tentacleCount / 2) * 15;
              const offsetY = spatEnemya.y + (i % 2 === 0 ? -20 : 20);

              graphics.beginPath();
              graphics.moveTo(offsetX, offsetY);

              for (let t = 0; t <= 1; t += 0.1) {
                const cx = Phaser.Math.Interpolation.Linear([offsetX, player.x], t);
                let cy = Phaser.Math.Interpolation.Linear([offsetY, player.y], t);

                // Oscilación distinta según fase
                if (fase === 1) {
                  cy += Math.sin(this.time.now / 200 + i + t * 5) * 40; // ligera
                } else {
                  cy += Math.sin(this.time.now / 200 + i + t * 5) * 10; // pesada
                }
        ...
        // --- Fase 1: rápido ---
        this.physics.moveToObject(player, spatEnemya, 800);

        // Cambiar a fase 2 después de 600ms
        this.time.delayedCall(600, () => {
          fase = 2;
          if (player && player.active && player.body && spatEnemya && spatEnemya.active) {
            this.physics.moveToObject(player, spatEnemya, 200); // lento
          }
        });
        ...
        // Lanzar proyectil y limpiar
        this.time.delayedCall(1500, () => {
          tentacleUpdate.remove();
          graphics.destroy();

          const zombieProyectil = enemigoZ.getFirstAlive(false);
          if (zombieProyectil) {
            zombieProyectil.setActive(true).setVisible(true);

            // Verificación antes de usar spatEnemya
            if (spatEnemya && spatEnemya.active && spatEnemya.body) {
              zombieProyectil.setPosition(spatEnemya.x, spatEnemya.y);
            } else {
              // enemigo ya no existe → no lanzar proyectil
              return;
            }

            zombieProyectil.isThrown = true;
            zombieProyectil.setTint(0xcc00cc);
            zombieProyectil.anims.play('ataque_Z', true);
            this.physics.moveToObject(zombieProyectil, player, 1400);

            this.time.delayedCall(500, () => {
              if (zombieProyectil.active && zombieProyectil.body) {
                zombieProyectil.body.stop();
                zombieProyectil.clearTint();
              }
            });
          }

          if (spatEnemya && spatEnemya.active) {
            spatEnemya.isAttacking = false;
            spatEnemya.estado = 'idle';
            spatEnemya.setFrame(5);
          }
        });

      } else if (!spatEnemya.isAttacking) {
        // --- PERSECUCIÓN NORMAL ---
        const direccion = player.x < spatEnemya.x ? -1 : 1;
        spatEnemya.body.setVelocityX(direccion * 120);
        spatEnemya.estado = 'corriendo';
        spatEnemya.anims.play(direccion === -1 ? 'enemy22_left' : 'enemy22_right', true);

        const distanciaX = Math.abs(player.x - spatEnemya.x);

        if (distanciaX < 200 && spatEnemya.body.blocked.down && spatEnemya.salto) {
          spatEnemya.body.setVelocityY(-400);
          if (spatEnemya.anims.currentAnim?.key !== 'enemy22_up') {
            spatEnemya.anims.play('enemy22_up', true);
          }
          spatEnemya.salto = false;
        }

        if (spatEnemya.body.blocked.down) {
          spatEnemya.salto = true;
        }
      }
    } else {
 ...
 updateEnemyLogic(enemigo10, spatEnemya, index) { 
    
    // Calcula la distancia entre el jefe y el seguidor
    const distance = Phaser.Math.Distance.Between(spatEnemya.x, spatEnemya.y, enemigo10.x, enemigo10.y);

    const chaseDistance = 500;
    const targetRadius = 150; // Distancia a la que deben rodear al jefe

    if (distance < chaseDistance) {
        // Pasa el índice y el radio a la función chase
        this.chase(enemigo10, spatEnemya, index, targetRadius); 
    
  }
  // Cambia la animación del enemigo en función de la dirección en la que se está moviendo
  if (enemigo10.body.velocity.x < 0) { // Si se está moviendo a la izquierda
    enemigo10.anims.play('enemy_left', true);
  } else if (enemigo10.body.velocity.x > 0) { // Si se está moviendo a la derecha
    enemigo10.anims.play('enemy_right', true);
  } else { // Si no se está moviendo
    enemigo10.anims.play('enemy_turn', true);
  }
}


chase(enemigo10, spatEnemya, index, targetRadius) {
    if (!spatEnemya) return; //  Correcto: Seguridad contra errores de null

    // 1. Calcular el ángulo de destino (distribución circular)
    const totalFollowers = enemigoZ.getChildren().length;
    const angle = (index / totalFollowers) * (2 * Math.PI); 

    // 2. Calcular la posición de destino única (punto en el círculo)
    const targetX = spatEnemya.x + targetRadius * Math.cos(angle);
    const targetY = spatEnemya.y + targetRadius * Math.sin(angle);

    // 3. Mover al enemigo hacia el punto de destino
    const dx = targetX - enemigo10.x;
    const dy = targetY - enemigo10.y;
    const angleToTarget = Math.atan2(dy, dx);

    // Mueve al enemigo hacia la posición de destino
    enemigo10.body.setVelocityX(enemigo10.speed * Math.cos(angleToTarget));
    enemigo10.body.setVelocityY(enemigo10.speed * Math.sin(angleToTarget));
}
  


updateEnemyLogic1(enemigo100, player) {
  const distance = Phaser.Math.Distance.Between(player.x, player.y, enemigo100.x, enemigo100.y);

  // Define una distancia de persecución y una distancia de evasión
  const chaseDistance = 700;
  

if (distance < chaseDistance) {
    // Si el jugador está dentro de la distancia de persecución, el enemigo lo persigue
    this.chase(enemigo100, player);
    
  }

  // Cambia la animación del enemigo en función de la dirección en la que se está moviendo
  if (enemigo100.body.velocity.x < 0) { // Si se está moviendo a la izquierda
    enemigo100.anims.play('enemy1_left', true);
  } else if (enemigo100.body.velocity.x > 0) { // Si se está moviendo a la derecha
    enemigo100.anims.play('enemy1_right', true);
  } else { // Si no se está moviendo
    enemigo100.anims.play('enemy1_turn', true);
  }
}
chase(enemigo100, player) {
  // Calcula la dirección hacia la que el enemigo debe moverse para alcanzar al jugador
  const dx = player.x - enemigo100.x;
  const dy = player.y - enemigo100.y;
  const angle = Math.atan2(dy, dx);

  // Mueve al enemigo hacia la posición del jugador a una velocidad determinada
  enemigo100.body.setVelocityX(enemigo100.speed * Math.cos(angle));
  enemigo100.body.setVelocityY(enemigo100.speed * Math.sin(angle));
}


updateEnemyLogic2(enemigoZa, player) {
  const distance = Phaser.Math.Distance.Between(player.x, player.y, enemigoZa.x, enemigoZa.y);
...
if (distance < chaseDistance) {
    // Si el jugador está dentro de la distancia de persecución, el enemigo lo persigue
    this.chase(enemigoZa, player);
    
  }

  // Cambia la animación del enemigo en función de la dirección en la que se está moviendo
  if (enemigoZa.body.velocity.x < 0) { // Si se está moviendo a la izquierda
    enemigoZa.anims.play('enemyA_left', true);
  } else if (enemigoZa.body.velocity.x > 0) { // Si se está moviendo a la derecha
    enemigoZa.anims.play('enemyA_right', true);
  } else { // Si no se está moviendo
    enemigoZa.anims.play('enemyA_turn', true);
  }
}
chase(enemigoZa, player) {
  // Calcula la dirección hacia la que el enemigo debe moverse para alcanzar al jugador
  const dx = player.x - enemigoZa.x;
  const dy = player.y - enemigoZa.y;
  const angle = Math.atan2(dy, dx);

  // Mueve al enemigo hacia la posición del jugador a una velocidad determinada
  enemigoZa.body.setVelocityX(enemigoZa.speed * Math.cos(angle));
  enemigoZa.body.setVelocityY(enemigoZa.speed * Math.sin(angle));
